    ...
    chainedm2m = function () {
        return {
            ...
            fill_field: function (val, initial_value, elem_id, url, initial_parent, auto_choose) {
                console.log('bindfields|initItem|chained|init|chained|fill_field');
                ...
                function trigger_chosen_updated() {
                    console.log('bindfields|initItem|chained|init|chained|fill_field|trigger_chosen_update');

                    if ($.fn.chosen !== undefined) {
                        $selectField.trigger('chosen:updated');
                        console.log('bindfields|initItem|chained|init|chained|fill_field||updated');
                ...
                if (!val || val === '') {
                    ...
                    console.log('bindfields|initItem|chained|init|chained|fill_field||null');
                    return;
                }
                ...
                $.getJSON(target_url, function (j) {
                    ...
                    $.each(j, function (index, optionData) {
                        ...
                        if (auto_choose || selected_values[optionData.value] === true) {
                            if ($selectedto.length) {
                                console.log('bindfields|initItem|chained|init|chained|fill_field||push|len');

                                selectedoptions.push(option);
                            } else {
                                console.log('bindfields|initItem|chained|init|chained|fill_field||push|len_else');

                                option.prop('selected', true);
                                options.push(option);
                            }
                        } else {
                            console.log('bindfields|initItem|chained|init|chained|fill_field|push|else');
                    ...
                    if ($selectedto.length) {
                        ...
                        for (i = 0, j = options.length; i < j; i = i + 1) {
                            ...
                            console.log('bindfields|initItem|chained|init|chained|fill_field|node' + $selectedto.length);
                    ...
                    // SelectBox.fireEvent(new Event("change"));
                });

            },

            init: function (chainfield, url, id, value, auto_choose) {
                console.log('bindfields|initItem|chained|init');
                var fill_field, val, initial_parent = $(chainfield).val(),
                    initial_value = value;

                if (!$(chainfield).hasClass("chained")) {
                    console.log('bindfields|initItem|chained|init|chained');
                ...
                $(chainfield).change(function () {
                    console.log('bindfields|initItem|chained|init|change');
